...
struct MemoryEditor {
    ...
    void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr)
    {
        ...
        if (PreviewDataType == ImGuiDataType_U64) {
            ImGui::SameLine();
            if (ImGui::Button("Goto")) {
                ImU64 addr = GetU64(DataPreviewAddr, mem_data) - base_display_addr;
    ...
    ImU64 GetU64(size_t addr, const ImU8* mem_data)
    {
        uint8_t buf[8];
        if (ReadFn)
            for (int i = 0, n = (int)8; i < n; ++i)
                buf[i] = ReadFn(mem_data, addr + i);
        else
            memcpy(buf, mem_data + addr, 8);
        uint64_t uint64 = 0;
        EndianessCopy(&uint64, buf, 8);
        return uint64;
    }
